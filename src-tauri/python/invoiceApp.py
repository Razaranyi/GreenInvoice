...
def get_cli_args():
    ...
    parser.add_argument('command', choices=['checkClient', 'preview', 'generate', 'addClient'], help='Command to execute')
    parser.add_argument('--file', default=None, help='Path to the input file')
    parser.add_argument('--client', default=None, help='Client name to add')
...
class InvoiceApp:
    ...
    def __init__(self, command, file_path=None):
        ...
        # Only initialize file if not addClient command
    ...
    def run(self):
        if self.command == 'addClient':
            client_name = self.client_name
            if not client_name:
                self.logger.error("No client name provided")
                return "Error: No client name provided"
            self.handle_missing_clients([client_name])
            return f"Added client: {client_name}"
    ...
    def __read_cred(self, file_path=None):
        try:
            if file_path is None:
                # Get the directory where the script is located
                script_dir = os.path.dirname(os.path.abspath(__file__))
                file_path = os.path.join(script_dir, "Samples", "Credentials.yml")
            ...
            if not key or not secret:
                self.logger.error("Missing key or secret in credentials file")
                exit(-1)

            return key, secret
        except FileNotFoundError as fileNotFoundErr:
            self.logger.error(f"Error in opening cred file: {fileNotFoundErr}")
            print(f"Could not find credentials file at: {file_path}")
            exit(-1)
        except Exception as e:
...
if __name__ == "__main__":
    print("Starting invoiceApp.py...")
    args = get_cli_args()
    print(f"Command line arguments: command={args.command}, file={args.file}, client={args.client}")
    
    # Create app instance with client name if it's an addClient command
    if args.command == 'addClient':
        app = InvoiceApp(command=args.command)
        app.client_name = args.client
    else:
