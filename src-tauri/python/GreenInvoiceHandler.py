...
class GreenInvoiceHandler:
    ...
    def add_client(self, client_names):
        """
        Add a new client to Green Invoice
        :param client_names: List of client names to add
        """
        headers = {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + self.JWT
        }
        end_point = "/clients"  # Remove the /api/v1 prefix as it's already in BASE_URL
        
        # Create the client data in the correct format
        client_data = {
            "name": client_names[0],  # Send the first client name as a string, not a list
            "active": True,
            "send": True,
            "country": "IL"
        }
        
        self.__send_POST_request(headers, end_point, client_data, "add client")
